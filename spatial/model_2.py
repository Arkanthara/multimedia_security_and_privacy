...
-----------------------------------------
    synchronise(
        residual,                # float32 spatial residual, shape (H, W)
        patch_size=32,
        key=0,
        tile_mode="symmetric",
        upsample_factor_rs=10,   # sub-pixel precision for rotation/scale
        upsample_factor_t=10,    # sub-pixel precision for translation
        upsample_factor_patch=2, # pixel-block size used at embed time
    ) → aligned_residual of shape (H', W'), float32

    The returned residual is geometrically aligned (rotation, scale,
    translation corrected) and NaN-padded where data were unavailable.
    Pass it directly to ``extract_bits_from_spatial``, which handles
    tiling accumulation and bit sampling internally.
"""
...
class WatermarkModel:
    ...
    def decode(
        ...
        # 5. Synchronise: correct rotation, scale and translation so the
        #    residual is aligned to the reference patch grid.
        #    Returns a float32 array of shape (H', W'), NaN-padded where
        #    data were unavailable after geometric correction.
        n_bits = self._n_embedded_bits()
        msgs = []
        confidences = []

        # Baseline: extract directly from the (unaligned) residual as a fallback
        msg, conf = extract_bits_from_spatial(
            residual, self.patch_size, n_bits, self.key,
            tile_mode=self.tile_mode, upsample_factor=self.upsample_factor,
        )
        msgs.append(msg)
        ...
        try:
            aligned_residual = synchronise(
                ...
                upsample_factor_patch=self.upsample_factor,
            )

            # Try the aligned residual and its three axis-flip variants to
            # handle residual 180° / reflection ambiguities in phase correlation.
            list_results = [
                aligned_residual,
                aligned_residual[::-1],
                aligned_residual[:, ::-1],
                aligned_residual[::-1, ::-1],
            ]
            ...
            print(f"Synchronisation failed, using unaligned residual: {e}")

        maximum_confidence_index = np.argmax(confidences)
        bits = msgs[maximum_confidence_index]

        # 6. Post-process: majority vote + LDPC BP decode
